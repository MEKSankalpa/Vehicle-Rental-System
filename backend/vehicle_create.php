...
if(empty($vehicle_title) || empty($vehicle_category) || empty($price_per_day) ||empty($brand_name) ||empty($model_year) || empty($fluel_type) || empty($seat_capacity) ||empty($description) || empty($_FILES["img1"]["name"]) ||empty($engine_capacity) ||empty($transmission) ){
    echo "System Error!";
    exit();
}else{
   ...
   $sql = "INSERT INTO vehicles(vehicle_title,vehicle_category,price_per_day,brand_name,model_year,fluel_type,engine_capacity,transmission,seat_capacity,description,image_name,airconditioner,powerdoorlocks,anti_lock_brake,brakeassist,powersteering,driverairbag,passenger_air_bag,powerwindow,cdplayer,central_locking,crashcensor,leather_seats) VALUES ('$vehicle_title', '$vehicle_category', '$price_per_day','$brand_name','$model_year','$fluel_type','$engine_capacity','$transmission','$seat_capacity','$description','$image_name','$airconditioner','$powerdoorlocks','$anti_lock_brake','$brakeassist','$powersteering','$driverairbag','$passenger_air_bag','$powerwindow','$cdplayer','$central_locking','$crashcensor','$leather_seats') ";
